...
use secp256k1::{KeyPair, Secp256k1, XOnlyPublicKey,SecretKey};
...
impl INode for Keyl {
    fn init(base: Base<Node>) -> Self {
        godot_print!("Hello, keyl!"); // Prints to the Godot console
...
pub fn to_npub(&self, secret: PackedByteArray) -> GString {
    ...
    fn hex_npub(&self, npub: GString) -> GString {
        ...
        match NostrXOnlyPublicKey::from_bech32(&s) {
            Ok(public_key) => {
                godot_print!("✅ Clave pública hex: {}", public_key.to_hex());
                public_key.to_hex().into()
            }
            Err(e) => {
